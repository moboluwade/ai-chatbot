...
const Chat = ({ chatTrain, setChatTrain }) => {
  ...
  return (
    <div className='bg-blue-gray-50 overflow-y-hidden pb-4 max-h-full h-full'>
      {chatTrain.map((chat) => { if(chat ===null){console.log(null)} else{return chat.role === 'user' ? <UserChat /> : <AIChat />}  })}
    </div>
  )
}
