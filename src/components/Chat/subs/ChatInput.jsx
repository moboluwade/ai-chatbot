...
const ChatInput = ({ chatInput, setChatInput, onSend }) => {
    ...
    return (
        <div className='sticky bottom-0 bg-white py-2 px-10 pl-5 w-full min-h-[3rem] flex flex-row items-end justify-between'>
            <textarea
                ...
                onInput={(inputRef) => { inputRef.currentTarget.style.height = 'auto'; inputRef.currentTarget.style.height = inputRef.currentTarget.scrollHeight + 'px' }}
                className='w-full min-h-8 max-h-32 pl-2 pt-2 overflow-auto resize-none border-2 border-opacity-50 border-blue-gray-600 rounded-md focus:outline-none scroll overscroll-contain'
                name="user-chat"
                id="user-chat-input"
                value={chatInput}>
            </textarea>
